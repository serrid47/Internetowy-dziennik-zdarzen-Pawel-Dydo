...
from .functions import check_permission
...
class LogCreate(LoginRequiredMixin, View):
    ...
    def get(self, request, name, page=1):
        if (check_permission('r', name, request.user)):
            ...
            latest_log_list = Log.objects.filter(user_changelog__changeLog__nazwa=name).order_by('-publication_date')[
                              (page - 1) * 10:page * 10]
...
class LogSearch(LoginRequiredMixin, View):
    def get(self, request, name, search_text, page=1):
        if (check_permission('r', name, request.user)):
            ...
            search_results_page = Log.objects.filter(user_changelog__changeLog__nazwa=name) \
                                      .filter(log_text__contains=search_text).order_by('-publication_date')[
                                  (page - 1) * 10:page * 10]
...
class ChangelogCreate(LoginRequiredMixin, View):
    def post(self, request):
        ...
        return render(request, 'changelog/changelog_form.html',
                      {'form': form, 'alert': 'Dziennik z taką nazwą już istnieje'})
...
class ChangelogSettings(LoginRequiredMixin, View):
    ...
    def get(self, request, name):
        if (check_permission('r', name, request.user)):
            ...
            return render(request, 'changelog/changelogSettings.html',
                          {'form_invite': form_invite, 'form_role': form_role,
...
class Changelogs(LoginRequiredMixin, ListView):
